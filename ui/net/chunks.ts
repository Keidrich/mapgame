...
export function chunkRoadsQuery(key: string): string {
  const bb = bboxOf(key);
  return `[out:json][timeout:40];(way["highway"~"^(${ROAD_TYPES.join('|')})$"](${bb});way["natural"="water"](${bb});way["waterway"="riverbank"](${bb});way["landuse"~"^(industrial|port|harbour)$"](${bb}););out geom;`;
}
...
async function fetchChunk(key: string, onStatus: (s: string) => void): Promise<GeoChunk> {
  ...
  const roads = parseRoads(await overpass(chunkRoadsQuery(key), onStatus), origin);
  if (roads.roads.length < 12) throw new Error('Not enough streets here');
  ...
  try { pois = parsePois(await overpass(chunkPoisQuery(key), onStatus)); } catch { /* businesses get invented */ }
  onStatus('Drawing blocks…');
  await new Promise(r => setTimeout(r, 20));
  const chunk = buildChunk({ key, roads: roads.roads, nodePos: roads.nodePos, water: roads.water, industrial: roads.industrial, pois: pois.pois, places: pois.places });
  if (chunk.blocks.length < 8) throw new Error('Not enough blocks here');
  return chunk;
}
