...
for (const style of STYLE_IDS) {
  ...
  console.log(`${style.padEnd(9)} worth ${String(m(r => r.w.history.at(-1)?.worth ?? 0)).padStart(8)} control ${String(m(r => select.controlShare(r.w) * 100)).padStart(5)}% heat ${String(m(r => r.w.player.heat)).padStart(5)} jobs ${String(m(r => (r.counts.jobs_done ?? 0) + (r.counts.jobs_failed ?? 0))).padStart(5)} made ${String(m(r => r.counts.made ?? 0)).padStart(4)} rats ${String(m(r => r.counts.rats_found ?? 0)).padStart(4)} coups ${String(m(r => r.counts.coups ?? 0)).padStart(4)} heirs ${String(m(r => r.w.player.generation - 1)).padStart(4)} over ${rs.filter(r => r.w.over).map(r => `${r.w.over!.ending}${r.w.over!.day}`).join(',') || '-'}`);
}
