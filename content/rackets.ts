...
export const OP_FAMILIES: Record<OpFamily, { label: string; icon: string; blurb: string }> = {
  wire: { label: 'The wire', icon: '📡', blurb: 'Work done at a keyboard, alone. Get inside somebody, use what you found, sell it on, and switch them off. Every job in this lane can be run with nobody but you.' },
};
...
export const OP_DEFS: Record<OpKind, OpDef> = {
  ...
  rat:             { label: 'Get Inside Their Business', icon: '🕳️', blurb: 'Their post, their calls, their standing arrangements. Learn what they are hiding — once, or for as long as you can keep it up.', family: 'wire', planDays: 1, minCrew: 0, maxCrew: 2, needs: { tech: 9, brains: 5 }, difficulty: 45, payout: [0, 0], heat: 4, target: 'npc', tier: 1,
    modes: [
  ...
  wire_fraud:      { label: 'Wire Fraud', icon: '🏧', blurb: 'Their arrangements, in your name, moved somewhere quiet. Only possible against somebody whose business you have already been inside.', family: 'wire', planDays: 3, minCrew: 0, maxCrew: 2, needs: { tech: 11, brains: 5 }, difficulty: 56, payout: [9000, 30000], heat: 16, target: 'npc', tier: 3, requires: { rattedTarget: true } },
  digital_strike:  { label: 'Pull Their Wires', icon: '🔌', blurb: 'Their tills stop ringing and their book stops balancing. Nobody gets hurt and nobody sees you.', family: 'wire', planDays: 1, minCrew: 0, maxCrew: 2, needs: { tech: 9, brains: 5 }, difficulty: 48, payout: [200, 1200], heat: 6, target: 'business', tier: 2, requires: { stance: ['beef', 'war'] } },
  ...
  long_con:        { label: 'The Long Con', icon: '🎩', blurb: 'Weeks of being somebody else to one person who has money and wants to believe you.', planDays: 5, minCrew: 0, maxCrew: 2, needs: { charm: 9, brains: 4 }, difficulty: 50, payout: [8000, 26000], heat: 5, target: 'npc', tier: 2, requires: { safehouseTier: 1 } },
  ...
  sim_swap:        { label: 'Take Their Number', icon: '📵', blurb: 'Their phone stops working and everything tied to it answers to somebody else for an afternoon. Only against somebody you have already been inside.', family: 'wire', planDays: 1, minCrew: 0, maxCrew: 2, needs: { tech: 9, charm: 4 }, difficulty: 42, payout: [1200, 5000], heat: 6, target: 'npc', tier: 1, requires: { rattedTarget: true } },
  ...
  stream_piracy:   { label: 'Pirate Feeds', icon: '📺', blurb: 'Somebody else\'s channels, resold by the month to half a district. Quiet money that arrives whether you get up or not.', family: 'wire', planDays: 2, minCrew: 0, maxCrew: 2, needs: { tech: 8, brains: 5 }, difficulty: 40, payout: [2000, 7000], heat: 5, target: 'none', tier: 2, requires: { safehouseTier: 1 } },
  betting_app:     { label: 'Run a Book Online', icon: '📲', blurb: 'The same book, on everybody\'s phone, with no room to stand in and no door to raid.', family: 'wire', planDays: 3, minCrew: 0, maxCrew: 3, needs: { tech: 9, brains: 6 }, difficulty: 50, payout: [4000, 13000], heat: 8, target: 'none', tier: 2, requires: { racketKinds: ['bookmaking', 'gambling_den', 'policy_bank'] } },
  synth_identity:  { label: 'Build a Person', icon: '📇', blurb: 'A name that belongs to nobody, with a history, a rating and a signature. Worth more the longer it sits untouched.', family: 'wire', planDays: 3, minCrew: 0, maxCrew: 2, needs: { tech: 9, brains: 6 }, difficulty: 52, payout: [3500, 12000], heat: 7, target: 'none', tier: 2, requires: { priorOps: ['shell_company', 'wire_fraud'] } },
