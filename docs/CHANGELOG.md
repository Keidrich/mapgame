...
and audits that catch them; specialists hired through the fixer for one job; a guaranteed fixer in
every city; a "Next" lead on the map pointing at a real person or place; three saved cities with
continue and delete on the start screen; days that pass while the app is closed.

**Why.** Three things `docs/REMAKE.md` §8 listed as not yet carried over from the original (street
crews, lieutenant audits, specialists, idle days, save slots), plus the one thing a new player feels
first: what to do next. And a real bug found while writing the leads: **the fixer's daily washing
window was never reset**, so after a day or two the fixer refused for the rest of the game.

**How.** `remake/sim/streetcrews.ts` (generation from its own rng stream, daily growth and skim, the
promotion to an outfit at 14 members, three new scenes on the boss). Skimming in `tick.ts`
(`skimmer`, `skimRate`) and an `audit` action with its odds on the button (`auditOdds`). `SPECIALISTS`
in `remake/content/world.ts`, `hire_specialist`, and team skill that counts them. `ensureFixer` and
`migrate` in `generate.ts`. `select.leads`. Save slots and idle days in `remake/ui/store.ts`.

**Heat, rebalanced.** A soak showed an empire of two dozen rackets living at heat 90 with nothing
left to do about it. Daily racket heat is 0.08 × heat × level (was 0.12), and each captain on the
payroll now takes 1.5 a day off *your* heat, not just the precinct's attention — the glossary line
on the captain says so.

**Numbers.** `npm run sim2 -- 60`: seed 7 $59k worth, 10.8% control, heat 76 on day 60; seed 1 $162k,
24.9%, heat 60; seed 2 $132k, 24.2%. The bot deals with every crew near its ground, so none became
an outfit in those runs, and the coverage table gained street crews, audits and specialists. The
bot plays loud — it runs every job it can staff — so its heat is the top of the range a careful
player sees. Remake tests: 77.

**Watch out.**

- **No save was dropped.** New world fields are generated by `migrate` on load from their own
  streams, so a city from the first release opens with exactly the crews and fixer a new game with
  its seed would have. `WORLD_VERSION` stays 1.
- The idle catch-up plays up to three days on its own. If that is ever unwelcome, `IDLE_MAX_DAYS`
  in `remake/ui/store.ts` is the knob.
- Still not carried over, and written down in `docs/REMAKE.md` §9: per-person kit, hostages beyond
  ransom, the Commission, landmark set-pieces, an admin panel for the bot.
