...
import { addHeat, adjustRel, bumpLoyalty, clamp, fearNote, gainFear, heatNote, log, loseHeat, loyaltyNote, money, spreadRep } from './util';
...
export function resolveProductionEvent(w: World, e: GameEvent, opt: string, rng: Rng): boolean {
  ...
  switch (`${e.kind}:${opt}`) {
    ...
    case 'bad_batch:sell': { if (sh) { const h = addHeat(w, 4, sh.blockId); spreadRep(w, sh.blockId, { trust: -3, respect: -2 }); log(w, `It sells. Then people start asking who sold it, and the block trusts you less.${heatNote(h)}`, 'bad', e.refs); } return true; }
    case 'worker_skims:confront': { if (n?.crew) { if (muscleCheck()) { const ly_10m = bumpLoyalty(n, -10); adjustRel(w, n, { fear: 15 }, 'backed'); log(w, `${n.name} stops.${loyaltyNote(ly_10m)} (+fear)`, 'good', e.refs); } else { p.crewIds = p.crewIds.filter(id => id !== n.id); for (const pr of Object.values(w.productions)) if (pr.workerId === n.id) pr.workerId = undefined; n.crew = undefined; n.role = 'patron'; n.rel.trust = -50; const h = addHeat(w, 6); log(w, `${n.name} quits on the spot and tells a cop where the safehouse is.${heatNote(h)}`, 'bad', e.refs); } } return true; }
    case 'worker_skims:replace': { if (n?.crew) { for (const pr of Object.values(w.productions)) if (pr.workerId === n.id) pr.workerId = undefined; n.crew.assignment = undefined; n.crew.status = 'idle'; const ly_8m = bumpLoyalty(n, -8); log(w, `${n.name} is off it. Somebody else will have to run it.${loyaltyNote(ly_8m)}`, 'info', e.refs); } return true; }
    case 'worker_skims:ignore': { if (n?.crew) { const ly_8 = bumpLoyalty(n, 8); n.notes.push('skims product'); log(w, `You let it go. ${n.name} keeps a little back every day.${loyaltyNote(ly_8)}`, 'info', e.refs); } return true; }
    case 'worker_snitch:lawyer': { if (n?.crew) { const h = loseHeat(w, 6); n.crew.statusDays = Math.max(1, Math.round(n.crew.statusDays * 0.5)); const ly = bumpLoyalty(n, 25); log(w, `A better lawyer, a shorter stretch, and ${n.name} forgets what they were going to say.${heatNote(h)}${loyaltyNote(ly)}`, 'money', e.refs); } return true; }
    case 'worker_snitch:move': { if (sh) { for (const id of sh.productionIds) w.productions[id].disrupted = Math.max(w.productions[id].disrupted, 3); w.blocks[sh.blockId].heat = clamp(w.blocks[sh.blockId].heat - 20); const h = loseHeat(w, 4); log(w, `Everything at ${sh.name} goes into a van for three days. Whatever ${n?.name ?? 'they'} described is not there any more.${heatNote(h)}`, 'info', e.refs); } return true; }
    case 'worker_snitch:message': { if (n?.crew) { if (muscleCheck()) { n.crew.loyalty = clamp(n.crew.loyalty + 5); adjustRel(w, n, { fear: 30 }, 'violence'); p.fear = clamp(p.fear + 2); log(w, `${n.name} gets a visitor in the yard and stops talking.`, 'warn', e.refs); } else { const h = addHeat(w, 10); log(w, `The visit goes wrong and now the guards know your name too.${heatNote(h)}`, 'bad', e.refs); } } return true; }
    ...
    case 'sabotage:hitback': { if (f) { f.standing[PLAYER] = clamp(f.standing[PLAYER] - 15, -100, 100); const h = addHeat(w, 6); if (muscleCheck()) { f.soldiers = Math.max(0, f.soldiers - 1); const fear = gainFear(w, 5); log(w, `One of ${f.short}'s people goes to the hospital. Everybody understands.${fearNote(fear)}${heatNote(h)}`, 'good', e.refs); } else { const c = p.crewIds.map(id => w.npcs[id]).find(c => c.crew?.status === 'idle' || c.crew?.status === 'assigned'); if (c?.crew) { c.crew.status = 'injured'; c.crew.statusDays = 4; c.crew.assignment = undefined; } log(w, `It goes badly. ${c ? `${c.name} is hurt.` : ''}${heatNote(h)}`, 'bad', e.refs); } } return true; }
    ...
    case 'shortage:find': { const kinds = Object.keys(market(w).shortage) as ProductionKind[]; if (charmCheck()) { for (const k of kinds) delete market(w).shortage[k]; log(w, 'A cousin of a cousin. The new supplier is dearer by nothing.', 'good'); } else { const h = addHeat(w, 5); log(w, `You ask around too loudly.${heatNote(h)}`, 'bad'); } return true; }
    case 'shortage:steal': { const c = idle(); const pr = sh ? w.productions[sh.productionIds[0]] : undefined; if (c && pr) { pr.stock += 5; const h = addHeat(w, 7, sh!.blockId); log(w, `${c.name} takes a van from a loading dock. Five days of stock, and a report filed.${heatNote(h)}`, 'warn', { npcId: c.id }); } else log(w, 'Nobody free to do it.', 'warn'); return true; }
