...
import { addHeat, addInfluence, adjustRel, bleedRel, bumpLoyalty, bumpStanding, clamp, factionOf, fearNote, gainFear, heatNote, log, loseHeat, loyaltyNote, money, nid, spreadRep } from './util';
...
export function resolveEventOption(w: World, e: GameEvent, opt: string, rng: Rng) {
  ...
  switch (key) {
    ...
    case 'quiet_asking:quiet': { for (const id of p.racketIds) { const r = w.rackets[id]; if (r && e.refs.blockId && w.businesses[r.businessId]?.blockId === e.refs.blockId) r.disrupted = Math.max(r.disrupted, 2); } const h = loseHeat(w, 8); for (const a of Object.values(w.authorities ?? {})) { a.attention = clamp(a.attention - 6, 0, 100); a.posture = postureFor(a.attention); } log(w, `Everything on those blocks goes quiet for a couple of days. So does the asking.${heatNote(h)}`, 'info', e.refs); break; }
    case 'quiet_asking:ignore': { if (rng.chance(0.5)) { const h = addHeat(w, 10); for (const a of Object.values(w.authorities ?? {})) { a.attention = clamp(a.attention + 14, 0, 100); a.posture = postureFor(a.attention); } log(w, `They found somebody who would talk, and the building is paying attention now.${heatNote(h)}`, 'bad', e.refs); } else log(w, 'Nobody tells them anything. The asking stops on its own.', 'good', e.refs); break; }
    ...
    case 'claim_questions:scare': if (e.refs.blockId) { const b = w.blocks[e.refs.blockId]; if (muscleCheck()) { p.fear = clamp(p.fear + 3); log(w, `The clipboard goes away and does not come back.`, 'good', e.refs); } else { const h = addHeat(w, 8, b?.id); log(w, `He files something. Somebody at the city now has ${b?.name ?? 'that block'} on a list.${heatNote(h)}`, 'bad', e.refs); } } break;
    ...
    case 'crew_raise:promise': if (n?.crew) { if (charmCheck()) { n.crew.loyalty = clamp(n.crew.loyalty + 5); log(w, `${n.name} buys it. For now.`, 'info', e.refs); } else { const ly = bumpLoyalty(n, -15); log(w, `${n.name} has heard that before.${loyaltyNote(ly)}`, 'bad', e.refs); } } break;
    case 'crew_raise:refuse': if (n?.crew) { const ly = bumpLoyalty(n, -12); log(w, `${n.name} says nothing.${loyaltyNote(ly)}`, 'warn', e.refs); } break;
    case 'cop_taste:pay': { const h = loseHeat(w, 12); log(w, `The sergeant is suddenly very busy elsewhere.${heatNote(h)}`, 'money', e.refs); break; }
    case 'cop_taste:refuse': { const h = addHeat(w, 8); log(w, `The sergeant makes a note.${heatNote(h)}`, 'bad', e.refs); break; }
    case 'betrayal:cut': if (n?.crew) { p.crewIds = p.crewIds.filter(id => id !== n.id); n.crew = undefined; n.role = 'patron'; n.rel.trust = -60; const h = addHeat(w, 6); log(w, `${n.name} is out. They know things.${heatNote(h)}`, 'warn', e.refs); } break;
    case 'betrayal:hunt': if (n?.crew) { p.crewIds = p.crewIds.filter(id => id !== n.id); if (muscleCheck()) { n.alive = false; n.crew.status = 'dead'; p.fear = clamp(p.fear + 8); const h = addHeat(w, 8); for (const c of p.crewIds.map(id => w.npcs[id])) if (c.crew) c.crew.loyalty = clamp(c.crew.loyalty + 5); log(w, `${n.name} is not talking to anyone any more. Your crew got the message.${heatNote(h)}`, 'warn', e.refs); } else { n.crew = undefined; n.role = 'patron'; n.rel.trust = -90; const h = addHeat(w, 15); log(w, `${n.name} got away and went straight to the precinct.${heatNote(h)}`, 'bad', e.refs); } } break;
    ...
    // The block's fear is `spreadRep`'s to decide — it is capped per person by what the act cost
    // and by how well they know you — so the line says what happened rather than a figure nobody
    // in particular got. The player's own fear and the heat are exact, so those are printed.
    case 'debtor:break': if (n) { adjustRel(w, n, { fear: 40, trust: -40 }, 'violence'); spreadRep(w, n.homeBlockId, { fear: 8 }, 2, 'violence'); const h = addHeat(w, 6, n.homeBlockId); const f = gainFear(w, 4); log(w, `${n.name} will walk with a limp. The block is quieter now.${fearNote(f)}${heatNote(h)}`, 'warn', e.refs); } break;
    ...
    case 'reporter:bribe': { const h = loseHeat(w, 15); log(w, `The story runs about somebody else.${heatNote(h)}`, 'money'); break; }
    case 'reporter:scare': if (muscleCheck()) { const h = loseHeat(w, 8); const f = gainFear(w, 3); log(w, `The reporter takes a job in another city.${fearNote(f)}${heatNote(h)}`, 'warn'); } else { const h = addHeat(w, 15); log(w, `The reporter wrote about the threat. Front page.${heatNote(h)}`, 'bad'); } break;
    case 'reporter:ignore': { const h = addHeat(w, 10); log(w, `The story runs.${heatNote(h)}`, 'bad'); break; }
    ...
    case 'skimming:slide': if (n?.crew) { const ly = bumpLoyalty(n, 12); const r = e.refs.racketId ? w.rackets[e.refs.racketId] : undefined; p.dirty = Math.max(0, p.dirty - Math.round((r?.lastIncome ?? 200) * 2)); log(w, `${n.name} keeps skimming and starts to like you for it.${loyaltyNote(ly)}`, 'info', e.refs); } break;
    ...
    case 'cops_sniffing:pay': { const r = e.refs.racketId ? w.rackets[e.refs.racketId] : undefined; const h = loseHeat(w, 6); if (r) w.blocks[w.businesses[r.businessId].blockId].heat = clamp(w.blocks[w.businesses[r.businessId].blockId].heat - 15); log(w, `The detective finds somewhere else to sit.${heatNote(h)}`, 'money', e.refs); break; }
    ...
    case 'crew_beef:heads': { const pair = crewPair(w, e); if (!pair) break; const ok = muscleCheck(); const moved = pair.map(c => bumpLoyalty(c, ok ? 8 : -8)); const same = moved.every(m => m === moved[0]); log(w, (ok ? 'Two bruised egos and a quiet crew.' : 'They both think you picked the other side.') + (same ? `${loyaltyNote(moved[0])} each` : ''), ok ? 'good' : 'bad', e.refs); break; }
    case 'lt_skim:audit': if (n?.crew) { const skim = n.crew.skim ?? 0; if (brainsCheck()) { const back = Math.round(skim * 0.6); p.dirty += back; n.crew.skim = 0; const ly_10m = bumpLoyalty(n, -10); log(w, `It is ${n.name}. About ${money(skim)} over the last while. You get ${money(back)} back and they know you are watching.${loyaltyNote(ly_10m)}`, 'bad', e.refs); } else { n.crew.loyalty = clamp(n.crew.loyalty - 3); log(w, `You cannot make the numbers say anything. ${n.name} watches you try.`, 'info', e.refs); } } break;
    case 'lt_skim:confront': if (n?.crew) { const skim = n.crew.skim ?? 0; if (muscleCheck()) { p.dirty += skim; n.crew.skim = 0; const ly_20m = bumpLoyalty(n, -20); log(w, `${n.name} goes white and brings back ${money(skim)} in a shoebox.${loyaltyNote(ly_20m)}`, 'good', e.refs); } else { p.crewIds = p.crewIds.filter(id => id !== n.id); n.crew = undefined; n.role = 'patron'; n.rel.trust = -60; n.grudge = { since: w.day, reason: 'you accused them', spread: 0 }; log(w, `${n.name} laughs, and is gone by morning with ${money(skim)} and the district's book.`, 'bad', e.refs); } } break;
    case 'lt_skim:slide': if (n?.crew) { n.crew.skim = 0; const ly_10 = bumpLoyalty(n, 10); log(w, `You say nothing. ${n.name} starts bringing the full count.${loyaltyNote(ly_10)}`, 'info', e.refs); } break;
    case 'lt_skim:demote': if (n?.crew) { n.crew.skim = 0; demote(n); const ly_15m = bumpLoyalty(n, -15); log(w, `${n.name} hands the book back without a word.${loyaltyNote(ly_15m)}`, 'warn', e.refs); } break;
    case 'lt_offer:raise': if (n?.crew) { n.crew.cut = Math.round(n.crew.cut * 1.4); n.crew.baseCut = n.crew.baseCut !== undefined ? Math.round(n.crew.baseCut * 1.4) : n.crew.baseCut; const ly_20 = bumpLoyalty(n, 20); if (f) f.standing[PLAYER] = clamp(f.standing[PLAYER] - 5, -100, 100); log(w, `${n.name} stays, at ${money(n.crew.cut)}/day. ${f?.short ?? 'They'} know they were turned down.${loyaltyNote(ly_20)}`, 'money', e.refs); } break;
    case 'lt_offer:lean': if (n?.crew) { if (muscleCheck()) { const ly_10m = bumpLoyalty(n, -10); adjustRel(w, n, { fear: 25 }, 'backed'); log(w, `${n.name} gets the message and stays. They will not forget how you said it.${loyaltyNote(ly_10m)} (+fear)`, 'warn', e.refs); } else if (f) { flipLieutenant(w, n, f); } } break;
